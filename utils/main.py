...
def _expand_rrule(component: Any, dtstart: datetime, window_start: datetime, window_end: datetime, tzname: str) -> List[datetime]:
...
def _download_image(session: requests.Session, url: str, dest_dir: Path, url_cache: Dict[str, str]) -> Optional[str]:
    """
    ...
    # Prefer a direct-ish Drive download URL if we have an id
    fetch_url = url
    if file_id:
    ...
    try:
        ...
        # Basic sanity check: do not save an html interstitial as an image
        if "text/html" in content_type.lower():
            # Try original URL as fallback if we swapped to uc?export=download
            if fetch_url != url:
    ...
    for component in cal.walk():
        ...
        for occ_start in starts:
            ...
            for img_url in images:
                saved = _download_image(session, img_url, EVENT_IMAGES_DIR, url_cache)
                if saved:
